...
//验证正整数
jQuery.validator.addMethod("checkPositiveInt",function(value,element){
	var reg = /^[1-9]([0-9])*$/;
	return this.optional(element) || reg.test(value);
},"无效的正整数");
...
jQuery.validator.addMethod("mobileOrTel", function(value,element) {
	var length = value.length;
	var mobileOrTel = /(^1[2-9]{1}[0-9]{9}$)|(^0[\d]{2,3}-?\d{7,8}$)|([\d]{1,3}-?\d{1,3}-?\d{4,8}$)/;
...
jQuery.validator.addMethod("minNotEq", function(value, element, param) { 
	return this.optional(element) || (NumInput.parseValue(value) > param);
}, "数值太小！");
