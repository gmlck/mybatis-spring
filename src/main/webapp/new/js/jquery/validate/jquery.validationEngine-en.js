		...
		newLang:function(){
			$.validationEngineLanguage.allRules = {
				...
				"fixedPhone":{
					...
					"alertText":"* 请填写电话号码格式010-12345678"
				},
				"email":{
				...
				"number":{
					// Number, including positive, negative, and floating decimal. credit:orefalo 保留两位小数或者一位
					"regex":/^([1-9]\d*)?[0-9](\.[0-9]{1,2})?$/,
					"alertText":"* 无效的整数或者小数,小数点后最多保留两位"
				},
				"date":{
				...
				"ajaxUserCall":{
					"url":"bms/checkProxyCode.do",
					// you may want to pass extra data on the ajax call
					"extraData":"name=eric",
					"alertText":"* 此编码已被其他人使用!",
					"alertTextLoad":"* 正在确认编码是否有其他人使用，请稍等。",
					"alertTextOk":"* 此编码可以使用!"
				},
				"ajaxUserCallPhp":{
					"url":"bms/checkCientCode.do",
					// you may want to pass extra data on the ajax call
					//"extraData":"name=eric",
				...
				"isImg":{
					"regex":/.(gif|jpg|jpeg|png)$/,
					"alertText":"* 图片格式不正确,目前支持(gif|jpg|jpeg|png)!"
				},
				"userNameCheck":{
					"regex":/^[a-zA-Z\u4e00-\u9fa5][\w\u4e00-\u9fa5\d]{2,29}$/,
					"alertText":"* 请输入3-30位以字母或中文开头的字符，不能含有特殊字符!"
				},
				//只能输入中文校验
				"onlyChinese":{
					"regex":/^[\u4e00-\u9fa5]+$/,
				...
				"onlyChinese30":{
					"regex":/^[\u4e00-\u9fa5]{1,30}$/,
					"alertText":"* 只能输入中文字符,最多输入30个!"
				},
				"onlyChineseNumber15":{
					"regex":/^[\u4e00-\u9fa5\d]{13,15}$/,
					"alertText":"* 只能输入中文和数字,输入范围13至15个字符!"
				},
				"ajaxCheckUserName":{
					"alertText":"* 此名称已被使用,请重新输入!",
					"alertTextOk":"* 此用户名可以使用!"
				},
				"ajaxCheckProxyCode":{
					"alertText":"* 此编码已被使用,请重新输入!",
					"alertTextOk":"* 此编码可以使用!"
				},
				"ajaxCheckClientCode":{
					"alertText":"* 此编码已被使用,请重新输入!",
				...
				"vehicleNum":{
				    ...
				    "alertText":"* 输入的车牌号格式不正确!"
				},
				"taxCard":{
					//isIDCard15=/^[1-9]\d{7}((0\d)|(1[0-2]))(([0|1|2]\d)|3[0-1])\d{3}00000$/;
					//isIDCard18=/^[1-9]\d{5}[1]\d{3}((0\d)|(1[0-2]))(([0|1|2]\d)|3[0-1])\d{4}00$/;
					//"regex":/^[0-9a-zA-Z]{24}$/,
					"alertText":"* 只能输入中文字符,最多输入30个!"
				},
				"checkMinute":{
				    "regex":/^([0-1][0-9]|[2][0-3]):([0-5][0-9])$/,
				    "alertText":"* 您输入的时间格式不对，请重新输入!"
				},
				"number15":{
					"regex":/^\d{15}$/,
				    "alertText":"* 格式不正确,15位数字!"
				},
				"number9":{
					"regex":/^\d{9}$/,
				    "alertText":"* 格式不正确,9位数字!"
				},
				"number12":{
					"regex":/^\d{12}$/,
				    "alertText":"* 格式不正确,12位数字!"
				}
			};
...
function eqSize(filed,rules,i,options){
		var eq = rules[i+1];
		var len = filed.val().length;
		if(len!=eq){
			var rule = options.allrules.eqSize;
			return rule.alertText + eq + rule.alertText2;
		}
}
function ajaxCheckUserName(filed,rules,i,options){
	var isPass = false;
		var filedValue = filed.val();
		if(filedValue == null || filedValue == "" || filedValue.length==0){
			return;
		}
//		load
		jQuery("#userName").validationEngine('showPrompt','正在检验此名称是否已被使用...','load',null,{promptPosition:"topRight"});
		$.ajax({
			url:"checkName.do",
			type:"post",
			data:{"fieldValue":filedValue},
			async:false,
			cache:false,
		...
		setTimeout(function(){
			jQuery('#userName').validationEngine('hide');
		},3000);
//		jQuery("#"+filed.id).validationEngine('hidePrompt');
		var rule = options.allrules.ajaxCheckUserName;
		if(!isPass){
...
//检查代理商编码是否重复
function ajaxCheckProxyCode(filed,rules,i,options){
	var isPass = false;
		var filedValue = filed.val();
		if(filedValue == null || filedValue == "" || filedValue.length<6){
			return;
		}
//		load
		jQuery("#infoUser_proxy_code").validationEngine('showPrompt','正在检验此编码是否已被使用...','load',null,{promptPosition:"topRight"});
		$.ajax({
			url:"bms/checkProxyCode.do",
			type:"post",
			data:{"fieldValue":filedValue},
...
function taxCard(filed,rules,i,options){
	var filedVal = filed.val();
	var taxCard15 = /^[0-9a-zA-Z]{24}[1-9]\d{7}((0\d)|(1[0-2]))(([0|1|2]\d)|3[0-1])\d{3}00000$/;
	var taxCard18 = /^[0-9a-zA-Z]{24}[1-9]\d{5}[1]\d{3}((0\d)|(1[0-2]))(([0|1|2]\d)|3[0-1])\d{4}00$/
	if(!(taxCard15.test(filedVal)||taxCard18.test(filedVal))){
		return rule.alertText;
	}
}
